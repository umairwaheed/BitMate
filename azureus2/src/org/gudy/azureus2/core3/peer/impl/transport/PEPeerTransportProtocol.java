...
implements PEPeerTransport
{
	...
	private Average averageHaveReceived; 
	...
	private boolean is_similar = true; // this field is used for the optimizations of the BitMate
	...
	{
		averageHaveReceived = Average.getInstance(1000, 30);
		manager = _manager;
	...
	public void sendOptimisticHave(final int pieceNumber, final int pieceOffset, final int pieceLength) {
		try {
			if (Logger.isEnabled())
				Logger.log(new LogEvent(this, stdOutLogId,
						"Sending optimistic have."));
	
			if (peerSupportsMessageType(BTMessage.ID_BT_OPTIMISTIC_HAVE) == false) return;
			if ( current_peer_state != TRANSFERING || pieceNumber == manager.getHiddenPiece()) return;
			if( peerHavePieces.flags[pieceNumber]) return;
			
			connection.getOutgoingMessageQueue().addMessage( new BTOptimisticHave( pieceNumber, pieceOffset, pieceLength), false );
	
			if (Logger.isEnabled())
				Logger.log(new LogEvent(this, stdOutLogId,
						"Optimistic have sent."));
			}
		catch(Throwable t)
		{}
  ...
  protected void decodeAZHandshake(AZHandshake handshake) {
	...
	protected void decodeHave( BTHave have ) {
		averageHaveReceived.addValue(1);
    ...
    protected void decodeOptimisticHave( BTOptimisticHave optimisticHave ) {
    	try {
	    	if (Logger.isEnabled())
	    		Logger.log(new LogEvent(this, stdOutLogId,
	    				"Starting to decode optimistic have."));
	
			final int number = optimisticHave.getPieceNumber();
			final int offset = optimisticHave.getPieceOffset();
			final int length = optimisticHave.getLength();
			optimisticHave.destroy();  
			
			int maxRequests;
			if (!this.isSnubbed()){
				if (!piecePicker.isInEndGameMode()){
					maxRequests = 2 + (int) (this.getStats().getDataReceiveRate() / 4096);
			...
			if ( this.isDownloadPossible() && this.getNbRequests() < maxRequests )
				this.request(number, offset, length);	
	
			if (Logger.isEnabled())
				Logger.log(new LogEvent(this, stdOutLogId,
						"Done decoding optimistic have."));
    	}
    	catch (Throwable t)
    	{
    ...
    if ( AEDiagnostics.CHECK_DUMMY_FILE_DATA ){
	  ...
	  if ( DEBUG_FAST ){
		...
		else
		{
			...
			if(!manager.isPipeliningEnabled()) return;
			List<PEPeer> peers = manager.getPeers();
			for(int i=0; i<peers.size(); i++) {
				PEPeerTransport pt = (PEPeerTransport)peers.get(i);
				if(pt != this){

					pt.sendOptimisticHave(pieceNumber, offset, length);
  ...
  getOutboundDataQueueSize() 
  {
	...
	public Double getAverageHaveReceived() {
		return averageHaveReceived.getDoubleAverage();
	}
