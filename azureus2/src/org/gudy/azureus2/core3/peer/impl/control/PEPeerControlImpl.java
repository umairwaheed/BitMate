...
DiskManagerCheckRequestListener, IPFilterListener
{
	...
	private static final LogIDs stdOutLogId = LogIDs.STDOUT;
	...
	private Average _averageHaveSent;
	private Average _averageTimeElapsed;
	private Average _averageDownloadRate;
	private int _pipeliningThreshold = 5120;
	...
	start()
	{
		...
		_averageHaveSent = Average.getInstance(1000, 30);
		_averageDownloadRate = Average.getInstance(1000, 10);
		_averageTimeElapsed = Average.getInstance(1000, 30);
	...
	{
		return _averageDownloadRate.getDoubleAverage() <= _pipeliningThreshold;
	...
	/**
	 * This function implements the Optimized Neighbor Selection of
	 * BitMate. The similar peers are updated after every 30 seconds.
	 */
	private void updateSimilarPeers() throws IOException {
		if ( (mainloop_loop_count % MAINLOOP_SIXTY_SECOND_INTERVAL) != 0 ){
			return;
		}
		
		final Double lowerLimit = _averageHaveSent.getDoubleAverage() - 1;
		final Double upperLimit = lowerLimit + 2; // effectively equal to _averageHaveSent.getDoubleAverage() + 1
		Double peerHaveAverage = 0.0;
		boolean similar = false;
		final ArrayList peer_transports = peer_transports_cow;
		final Double similarCountUpperLimit = 0.7 * peer_transports.size(); // need to maintain 70% similar nodes
		int similarCount = 0;
		...
		for (int i = 0; i < peer_transports.size(); i++) {
			...
			peerHaveAverage = pc.getAverageHaveReceived();
			if(peerHaveAverage <= upperLimit && peerHaveAverage > lowerLimit &&
					pc.isSeed() == false && pc.getPeerState() == PEPeer.TRANSFERING
					&& similarCount < similarCountUpperLimit) {
			...
			name = ReverseMap.fromAddress(pc.getIp());
			...
			if (answers.length == 0) {
			    ...
			    _averageTimeElapsed.addValue((endtime-starttime));
				if ( Math.abs((endtime-starttime) - _averageTimeElapsed.getDoubleAverage()) > 100.0){
			    	similar = false;
			    }
		    }	
			pc.setSimilarity(similar);
		}

		if (Logger.isEnabled())
			Logger.log(new LogEvent(this, stdOutLogId,
					"Updated the similar peers."));
	...
	public void schedule()
	{
		try{
			...
			updateSimilarPeers(); // BitMate Optimized Neighbor Selection.
...
//	send the have requests out
	...
	private void updateStats() {   
		...
		_averageDownloadRate.addValue(_stats.getDataReceiveRate());
