...
public class UnchokerUtil {
  ...
  public static ArrayList<PEPeer> getNextOptimisticPeers( ArrayList<PEPeer> all_peers, boolean factor_reciprocated, boolean allow_snubbed, int num_needed ) {
    ...
    /*
     * The following two blocks are the optimizations of BitMate.
     * The following two code blocks are the replicas of the original algorithm
     * with the addition of similarity check. If similar peers cannot be 
     * found we fall back to the original algorithm.  
     */
    for( int i=0; i<all_peers.size(); i++) {
    	PEPeer peer = all_peers.get(i);
    	if( isUnchokable( peer, false) && peer.isChokedByMe() && peer.getSimilarity()) {
    ...
    if( optimistics.isEmpty() && allow_snubbed ) {  //try again, allowing snubbed peers as last resort
        for( int i=0; i < all_peers.size(); i++ ) {
      	  PEPeer peer = all_peers.get( i );
          if( isUnchokable( peer, true ) && peer.isChokedByMe() && peer.getSimilarity() ) {
            optimistics.add( peer );
          }
        }
      }
	...
	if (Logger.isEnabled())
		Logger.log(new LogEvent(new UnchokerUtil(), stdOutLogId,
				"Smart Unchoke done successfully."));


    // Could not find the desired similar nodes so falling back to the original algorithm.
