...
//						startPriority +=(PRIORITY_W_RARE +peerControl.getNbPeers()) /avail;
	...
	private final int getRequestCandidate(final PEPeerTransport pt )
	{
		...
		if ( pt.getSimilarity() == false) {
			PEPeerControl manager = pt.getControl();
			final List<PEPeer> peers = manager.getPeers();
			int startIndex = peerHavePieces.start;
			int endIndex = peerHavePieces.end;
			boolean othersHaveIt = false;
			BitFlags similarPeerHave;

			for (int i =startIndex; i <=endIndex; i++) {
				final PEPiece pePiece = pePieces[i];
				
				if ( peerHavePieces.flags[i] && (pePiece == null || pePiece.isRequestable()) ) {
					if (pePiece !=null) {
					...
					for (int p = 0; p < peers.size(); p++) {
						...
						PEPeerTransport pc = (PEPeerTransport) peers.get(p);
						if(pc != pt && peers.get(p).getSimilarity()) {
							similarPeerHave = pc.getAvailable();
							if (similarPeerHave != null && similarPeerHave.flags[i]) {
								othersHaveIt = true;
								break;
							}
						}
					}
					if (othersHaveIt == false) {
						return i;
					}
				}				
			}
		...
		boolean		resumeIsRarest = false; // can the peer continue a piece with lowest avail of all pieces we want
